...
#### Q: 如何處理前端與後端的資料交換？
A:
- **請求 (Request)**：必須使用 `application/x-www-form-urlencoded` (Form Data)。React 中請使用 `URLSearchParams` 建構 payload。
- **回應 (Response)**：後端回傳標準 JSON 格式：`{success: true, data: ...}` 或 `{success: false, message: ...}`。
